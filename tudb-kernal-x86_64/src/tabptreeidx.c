...
ta_idx_node_t* taIndexCreateNode(ta_idx_t *bptree, unsigned char leaf,
	...
	node->id = getOneId(ta_id_fp, caches->taIdxIds, ID_INDEX_QUEUE_LENGTH);
...
static int _dealloc(ta_idx_node_t *node, FILE *ta_idx_id_fp, FILE *ta_idx_db_fp) {
	...
	fwrite(node->content, sizeof(unsigned char), ta_bptree_idx_node_bytes,
			ta_idx_db_fp);
	recycleOneId(node->id, caches->taIdxIds, TIMEAXIS_ID_QUEUE_LENGTH, ta_idx_id_fp);
	free(node->content);
	if (node->leaf == 1)
		free(node->tuIdxIds);
	else
...
static int _bptree_merge(ta_idx_t *bptree, ta_idx_node_t *left,
		ta_idx_node_t *right, int mid, FILE *ta_idx_id_fp, FILE *ta_idx_db_fp) {
	int m = 0;
...
//	}
	...
	deallocTaIndexNode(right, ta_idx_id_fp, ta_idx_db_fp);

	/* Check if need to merge further */
	if (parent->num < bptree->min) {
		return ta_bptree_merge(bptree, parent, ta_idx_id_fp, ta_idx_db_fp);
	}

	return 0;
}

int ta_bptree_merge(ta_idx_t *bptree, ta_idx_node_t *node, FILE *ta_idx_id_fp,
		FILE *ta_idx_db_fp) {
	int idx = 0, m = 0, mid = 0;
	...
	if (NULL == parent) {
		if (node->num == 1) {
			if (NULL != node->child[0]) {
				...
				deallocTaIndexNode(node, ta_idx_id_fp, ta_idx_db_fp);
			}
		}
		return 0;
	}
...
//		print_ta_index(ta_idx);
		...
		if ((node->num + left->num + 1) <= bptree->max) {
			return _bptree_merge(bptree, left, node, mid, ta_idx_id_fp,
					ta_idx_db_fp);
		}
...
//	print_ta_index(ta_idx);
	...
	if ((node->num + right->num + 1) <= bptree->max) {
		return _bptree_merge(bptree, node, right, mid, ta_idx_id_fp,
				ta_idx_db_fp);
	}
...
static int _bptree_delete_leaf(ta_idx_t *bptree, ta_idx_node_t *node, int idx,
		long long tuid, FILE *ta_idx_id_fp, FILE *ta_idx_db_fp) {
...
//	printf("--\n");
	...
	if (node->num < bptree->min) {
		return ta_bptree_merge(bptree, node, ta_idx_id_fp, ta_idx_db_fp);
	}
	return 0;
}
...
//	print_ta_index(ta_idx);
	...
	if (found == 1) {
		// update
		return _bptree_delete_leaf(bptree, node, idx, tuid, ta_idx_id_fp,
				ta_idx_db_fp);
	}
